    ...
    *buf_out = (char*)malloc(ip->size);
    ...
    if(n < NDIRECT)
    {
        ...
        for(i=0; i<NDIRECT;i++)
        {
            bn = ip->blocks[i];
            bm->read_block(bn,bp);
            memcpy(*buf_out + i * BLOCK_SIZE,bp ,BLOCK_SIZE);
            rsize += BLOCK_SIZE;
        }
        ...
        for(i=0; i<=n - NDIRECT;i++)
        {
            bn = blocks[i];
            bm->read_block(bn,bp);
            memcpy(*buf_out +(i+NDIRECT) * BLOCK_SIZE,bp ,MIN(BLOCK_SIZE,ip->size - rsize));
            rsize += BLOCK_SIZE;
        }
    }
  return;
   ...
   */
    ...
    if(n < NDIRECT)
    {
        ...
        for(i=0; i<NDIRECT;i++)
        {
            bn = bm->alloc_block();
            bzero(bp,BLOCK_SIZE);
            memcpy(bp,buf + i * BLOCK_SIZE,BLOCK_SIZE);
            bm->write_block(bn,bp);
            ip->blocks[i] = bn;
            wsize += BLOCK_SIZE;
        }
        ...
        for(i=0; i<=n-NDIRECT;i++)
        {
            bn = bm->alloc_block();
            bzero(bp,BLOCK_SIZE);
            memcpy(bp,buf + (i+NDIRECT) * BLOCK_SIZE,MIN(BLOCK_SIZE,size - wsize));
            bm->write_block(bn,bp);
            indblocks[i] = bn;
            wsize += BLOCK_SIZE;
        }
