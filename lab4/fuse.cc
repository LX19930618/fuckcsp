        ...
        struct fuse_file_info *fi)
{
    ...
    // Change the above line to "#if 1", and your code goes here
    size_t wsize;
    yfs->write(ino,size,off,buf,wsize);
        ...
        mode_t mode, struct fuse_entry_param *e)
{
    ...
    yfs_client::inum ino;
    yfs_client::status ret = yfs->create(parent,name,mode,ino,extent_protocol::T_FILE);
    struct stat st;
    getattr(ino,st);
    e->ino = ino;
    e->attr = st;
        ...
        off_t off, struct fuse_file_info *fi)
{
     ...
     */
    list<yfs_client::dirent> dirlist;
    yfs->readdir(inum,dirlist);
    list<yfs_client::dirent>::iterator lt;
    
    for ( lt = dirlist.begin( ); lt != dirlist.end( ); lt++ )
        dirbuf_add(&b,(*lt).name.c_str(),(*lt).inum);
        ...
        mode_t mode)
{
    ...
    // Change the above line to "#if 1", and your code goes here
    yfs_client::inum ino;
    yfs->create(parent,name,mode,ino,extent_protocol::T_DIR);
    struct stat st;
    getattr(ino,st);
    e.ino = ino;
    e.attr = st;
